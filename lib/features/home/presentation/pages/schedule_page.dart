...
class _SchedulePageState extends State<SchedulePage> {
  ...
  Future<void> _showClassDialog({ClassEntry? entry, int? index}) async {
    ...
    final codeCtrl = TextEditingController(text: entry?.code ?? '');
    final instrCtrl = TextEditingController(text: entry?.instructor ?? '');
    final locCtrl = TextEditingController(text: entry?.location ?? '');
    final unitsCtrl = TextEditingController(
      text: entry != null ? entry!.units.toString() : '',
    );
      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, dialogSetState) {
            return AlertDialog(
              title: Text(entry == null ? 'Add Subject' : 'Edit Subject'),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    final parsedUnits =
                        double.tryParse(unitsCtrl.text.trim()) ?? 0.0;
              ...
              itemBuilder: (context, i) {
                ...
                return Material(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      '${c.code.isNotEmpty ? c.code + ' • ' : ''}${c.instructor.isNotEmpty ? c.instructor + ' • ' : ''}${c.units > 0 ? c.units.toString() + ' units\n' : ''}${c.days.join(', ')} • ${c.start.format(context)} - ${c.end.format(context)}\n${c.location}',
                    ),
                    isThreeLine: true,
                    trailing: IconButton(
  ...
  Widget _buildTimetable() {
    ...
    if (_classes.isNotEmpty) {
      minStart = _classes
          .map((c) => c.start.hour + c.start.minute / 60.0)
          .reduce((a, b) => math.min(a, b));
      maxEnd = _classes
          .map((c) => c.end.hour + c.end.minute / 60.0)
                          ...
                          for (final c in _classes)
                            if (c.days.contains(days[di]))
                              Positioned(
                                top:
                                    topPadding +
                                    ((c.start.hour + c.start.minute / 60) -
                                            minStart) *
                                        rowHeight,
                                left: 0,
                                right: 0,
                                height: math.max(
                                  ((c.end.hour + c.end.minute / 60) -
                                          (c.start.hour +
                                              c.start.minute / 60)) *
                                      rowHeight,
                                  36.0,
                                ...
                                child: GestureDetector(
                                  onTap: () => _showClassDialog(
                                    entry: c,
                                    index: _classes.indexOf(c),
                                  ),
                                  child: Builder(
                                    builder: (context) {
                                      final bg = _colorForEntry(
                                        c,
                                      ).withOpacity(0.95);
                                      final textColor =
                                          bg.computeLuminance() > 0.55
                                          ? Colors.black
                                          : Colors.white;
                                      return Container(
